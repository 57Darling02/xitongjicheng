<template>
    <div class="login-header fade-item" style="--delay: 0.05s;">
        ...
        <div class="login-title">铁路购-注册</div>
    </div>
    <el-form class="login-content fade-item" style="--delay: 0.1s;max-width: 300px;margin:25px 25px;" :model="form"
        ...
        <el-form-item>
            <div style="flex: 1;justify-content: center;align-items: center;display: flex;">
                <el-button style="width: 60%;" type="primary" @click="onSubmit" plain round>注册</el-button>
            </div>
        </el-form-item>
        <el-form-item>
            ...
            已有账号？<el-link type="primary" @click="$router.push({ name: 'Login' })">返回登入</el-link>
            </p>
        </el-form-item>
...
const onSubmit = async () => {
    ...
    formRef.value.validate((valid: boolean) => {
        if (valid) {
            userStore.register(form.username, form.password, form.role)
            // window.open('/', '_self')
        } else {
